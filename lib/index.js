...
function _downloadModules(depList) {
    fsExtra.removeSync(path.join('./', componentsDir))
    return depList.map((mod, index) => {
            ...
            try {
                ...
                requestPromise({
                    ...
                    for (let dep of depList) {
                        ...
                        if (index !== -1) {
                            let mod = dependenciesMods[index]
                            usingComponents[mod.name] = `${mod.path}/${mod.name}`
                        }
                    }
